...
// 게시글 상세보기 화면 // DB에서 JSON으로 받아오면 페이지화
...
const blogPostData = {
  post_id: {
    title: "초록초록한 분위기의 집",
    content: "자연 속에 있는 힐링과 낭만이 있는 집입니다.",
    image_id: {
      0: ["Room 1"],
      1: ["Bathroom 1"],
      2: ["Room 1"],
    ...
    amenity_list: ["Oven", "TV", "Refrigerator", "Microwave", "Bed", "Lamp"],
  },
};
...
const BlogPost = ({ post }) => {
  return (
    <div className="blog-post">
      <h1>{post.title}</h1>
      <ImageList images={Object.values(post.image_id).flat()} />
      <p>{post.content}</p>
      <ul>
        {post.amenity_list.map((amenity, index) => (
...
function App() {
  return (
    <div className="App">
      ...
      <main>
        <BlogPost post={blogPostData.post_id} />
      </main>
    </div>
  );
}
...
const Ammenities = () => {
    ...
    return (
        <div>
            {/* {onLoading} */}
            <h1 style={{textAlign:'center'}}>titles</h1>
            thumb_img_path
            {/* <p style={{textAlign:'center'}}>숙소 어메니티 확인 화면입니다.</p> */}
            <Button variant="primary" type="submit" href="/become-host">이전</Button>
            <Button variant="primary" type="submit" href="">완료</Button>
